...
const Skills = () => {
  ...
  return <section id="skills" className="py-20 px-4 bg-slate-900/50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {skillCategories.map((category, index) => <Card key={index} className={`bg-gradient-to-br ${category.bgGradient} ${category.borderColor} hover:border-opacity-60 transition-all duration-300 hover:scale-105 hover:shadow-xl`}>
              <CardContent className="p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill, skillIndex) => <Badge key={skillIndex} variant="secondary" className={`bg-gradient-to-r ${category.gradient} text-white hover:scale-105 transition-transform duration-200 cursor-default`}>
                      {skill}
                    </Badge>)}
                </div>
              </CardContent>
        ...
        <div className="mt-16 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-5xl mx-auto">
            {skillCategories.flatMap(category => category.skills).map((skill, index) => <Badge key={index} variant="outline" className="text-slate-300 border-slate-600 hover:border-cyan-400 hover:text-cyan-400 transition-all duration-300 cursor-pointer text-sm py-2 px-4">
                {skill}
              </Badge>)}
          </div>
        </div>
