      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid gap-8">
          {researchPapers.map((paper) => (
            <Card key={paper.id} className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border-slate-700/50 hover:border-slate-600/50 transition-all duration-300">
              <CardHeader>
              ...
              <CardContent>
                <Collapsible open={expandedPaper === paper.id} onOpenChange={() => togglePaper(paper.id)}>
                  <CollapsibleTrigger asChild>
                    <Button variant="outline" className="w-full mb-4 border-slate-600 text-slate-300 hover:bg-slate-800/50">
                      <FileText className="w-4 h-4 mr-2" />
                      {expandedPaper === paper.id ? 'Hide Full Paper' : 'Read Full Paper'}
